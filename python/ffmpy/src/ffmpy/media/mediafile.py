...
class Media(object):
    ...
    def __init__(self, path: str | Path) -> None:
        ...
        if isinstance(path, Path):
            ...
            self._path = Path(path).expanduser()
        # reset probe object
        self.__probe: FFprobe | None = None

    def check(self, should_exist: bool, handler: ErrorHandler | None):
        """
    ...
    def __getattr__(self, name: str):
        ...
        try:
            ...
            error(
                f"{self} : tried to access property {name} : not found, {E} (have you tried to call `probe` first ?)"
            )
            return None
